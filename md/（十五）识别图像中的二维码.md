### 识别图像中的二维码
##### 二维码的特征
QRCode：有一维条码及其它二维条码所具有的信息容量大、可靠性高、可表示汉字及图象多种文字信息、保密防伪性强等优点</br>
###### 由左上，右上，左下三个点的矩形确认是否是一个矩阵二维码
###### 举证特点正中心 从左往右或从上往下黑白比例为（1:1:3:1:1）
![QRcode](https://github.com/TF27674569/OpenCV/blob/master/image/QR_code.png)
##### 思路
1. 灰度
2. 二值化
3. 找出所有轮廓
...
##### 使用ZBar识别二维码
###### ZBar的配置
1. [ZBar官网](http://zbar.sourceforge.net/download.html)选中Windows下载
2. 安装，全勾选（是一个解压过程）
3. [github](https://github.com/dani4/ZBarWin64) clone整个项目
4. 将clone下来的项目中lib文件夹（libzbar64-0.lib，libzbar64-0.pdb）拷贝到安装目录的lib文件夹里面
5. 将clone下来的ZBarWin64\zbar\libiconv\dll_x64里面的dll库拷贝到安装目录下的bin里面
6. 将clone下来的项目中lib文件夹（libzbar64-0.dll）拷贝到安装目录的bin文件夹里面
7. 配置vs包含目录（属性VC++目录，包含目录）这里选择安装目录的include目录
8. 配置vs库目录（属性VC++目录，库目录）这里选择安装目录的bin目录
9. 配置连接器（连接器，附加库目录）添加libzbar-0.lib，libzbar64-0.lib
10. 配置连接器（连接器，输入，附加依赖项）添加libzbar-0.lib，libzbar64-0.lib
